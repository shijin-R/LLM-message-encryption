...
- 当前未跟踪文件仅包含新增的 `uie-base` 目录说明文件；实际模型权重仍被 `.gitignore` 覆盖。
- 本地存在虚拟环境、uv 缓存、PaddleNLP 缓存、Python 缓存、wordtag 模型文件和 UIE 模型文件，均已被 `.gitignore` 忽略。
- 仓库已跟踪文件整体合理；实际模型权重没有进入 Git 跟踪，只保留 `resources/models/wordtag/README.md` 和 `resources/models/uie-base/README.md` 作为目录说明。
...
| `example_preprocess_request.json` | 综合预处理联调示例 |
| `tests/test_service.py` | 服务行为回归测试 |
| `resources/models/wordtag/README.md` | 模型目录说明，占位保留 |
| `resources/models/uie-base/README.md` | UIE 模型目录说明，占位保留 |
...
| `resources/models/uie-base/*` | UIE 模型权重、推理文件和缓存体积大，不适合进入 Git |
